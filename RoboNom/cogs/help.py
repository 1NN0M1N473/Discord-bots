...
class help(commands.Cog):
    ...
    async def help(self, ctx, argument: typing.Optional[str] = "None"):
        if (argument == "None"):
            ...
            embed = discord.Embed(title='RoboNom help', description=("Hi there {}! Here are the commands you can use with this bot:".format(ctx.message.author.mention)), color = 0xf08136)
            embed.add_field(name='_ _', value='_ _', inline=False)
            embed.add_field(name='rn?help', value='Displays usable commands', inline=True)
            embed.add_field(name='rn?ping', value='Measures your latency!', inline=True)
            embed.add_field(name='rn?quote', value='Fetches a random quote from the ZenQuote API', inline=True)
            embed.add_field(name='_ _', value='_ _', inline=False)
            embed.add_field(name='rn?nerdyquote', value="Fetches a random programming-themed quote", inline=True)
            embed.add_field(name='rn?robonom <say something>', value="Engage in conversation with the bot; responses not guaranteed to make sense", inline=True)
