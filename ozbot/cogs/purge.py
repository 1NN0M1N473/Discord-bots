...
class help(commands.Cog):
    ...
    async def clear(self, ctx, argument: typing.Optional[int], other = None):
        if argument == None:
            if other == None:
                pass
            elif other.lower() == all:
                argument = 1001
        amount = argument
        if amount != None:
            if amount <= 1000:
            ...
            await ctx.message.delete()
            await ctx.send("**[PURGE]** The argument must be a number or `all` !", delete_after = 5)
